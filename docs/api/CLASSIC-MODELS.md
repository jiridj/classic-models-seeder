...
The Classic Models CLI provides commands to update timestamps in the API database, making the demo data current for demonstrations and testing.

### Workflow

1. **Generate Updated Data**: Run `cmcli update` to transform the original dataset dates to current dates and generate JSON files
2. **Verify API Connection**: Run `cmcli api verify` to test connectivity and authentication
3. **Update API Database**: Run `cmcli api update-timestamps` to push the updated timestamps to the API
...
# Update all timestamps (orders and payments)
cmcli api update-timestamps

# Preview changes without updating
cmcli api update-timestamps --dry-run

# Update only orders
cmcli api update-timestamps --orders-only

# Update only payments
cmcli api update-timestamps --payments-only
```
...
**Note**: Rate limiting is disabled by default for local API instances. Enable it by setting `CLASSIC_MODELS_RATE_LIMIT` to a positive number (e.g., 100) if you need to throttle requests.
...
- Authenticates with the API using JWT tokens
- Updates each record via PATCH requests
- Provides progress tracking and error handling
- Reports summary statistics

### SSL Verification

By default, SSL certificate verification is enabled for security. For development environments with self-signed certificates, you can disable verification by setting `CLASSIC_MODELS_VERIFY_SSL=false` in your `.env` file.

**Warning**: Disabling SSL verification should only be done in trusted development environments, as it exposes the connection to potential man-in-the-middle attacks.
...
7. **Demo Data Management**: Update timestamps to keep demo data current